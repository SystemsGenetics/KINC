...
Installing KINC on Ubuntu 22.04
-------------------------------
...
.. code:: bash
  ...
  sudo apt install clinfo libboost-dev libgsl-dev \
		liblapacke-dev libopenblas-dev libopenmpi-dev \
		ocl-icd-opencl-dev qtcreator qtbase5-dev qt5-qmake \
    cmake
...
Install KINC.R
~~~~~~~~~~~~~~
The KINC.R package provides supplementary R functions to assist with analysis of files used and generated by KINC. Therefore, some steps in the KINC workflow require KINC.R. To fully use KINC you should install KINC.R. 

To install KINC.R we need to use the `devtools` package in R. In Ubuntu 22.04 This package requires a additional packages be installed:
...
Now, in R we can install the `devtools` package:

.. code:: bash

  install.packages('devtools')

Now, we can install KINC.R
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
To visualize networks created using KINC, the software comes with an interactive 3D Dash based python viewer.  Using your favorite Python package manager install the following libraries:
