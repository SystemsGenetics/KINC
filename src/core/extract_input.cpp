...
QVariant Extract::Input::data(int index, Role role) const
{
   ...
   case CSMRSquareFilter:
      ...
      case Role::WhatsThis: return tr("This is only used if a Condition-Specific Martrix is provided above and applies to quantitative and ordinal tests. This filters the network such that only edges (clusters) with r-squared values from liner regression testing above the given values are kept. Provide a single r-squared value to filter all features with the same value. However, you can specify different r-squared values for different features. For example, suppose you were tesing a feature named 'Weight' and you wanted edges with an r-squared value > 0.5, you would input \"Weight,0.5\". You can also filter globaly, if you want any edges with a r-squared value below 1e-3 you would input \"1e-3\". Additionally you can filter for greater than the value with \"gt,1e-3\" or \"Weight,gt,1e-3\".Just note, when using more than one filter seperate them by two colons \"::\" for example \"Weight,lt,1e-3::Height,gt,1e-3\".");
      case Role::Default: return "";
      default: return QVariant();
      }
   default: return QVariant();
